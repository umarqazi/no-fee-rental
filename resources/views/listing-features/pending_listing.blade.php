...
<div class="listing-wrapper">
    @foreach($listing->pending as $pl)
        <div class="listing-row">
            ...
            <div class="info">
                <p class="title">{{ str_limit(is_exclusive($pl->listingTypes) ? $pl->street_address : $pl->display_address, STR_LIMIT_LIST_VIEW, ' ...') }}</p>
        ...
        @foreach($listing->pending as $pl)
            <div class="col-lg-3 col-md-4 col-sm-6">
                <div class="listing-thumb">
                    <img src="{{ asset(!empty($pl->thumbnail) ? $pl->thumbnail : DLI) }}" alt="" style="height: 205px; width: 100%;" class="main-img" />
                    <div class="info">
                        <p class="title">{{ str_limit(is_exclusive($pl->listingTypes) ? $pl->street_address : $pl->display_address, STR_LIMIT_GRID_VIEW, ' ...') }}</p>
                        <p><i class="fa fa-tag"></i> ${{ $pl->rent }}</p>
                        <p>Freshness Score : 90%</p>
                        <ul>
                            <li><i class="fa fa-bed"></i> {{ str_formatting($pl->bedrooms, 'Bed') }}</li>
