...
$(() => {
    ...
    $body.on('click', '.remove-single', async function (e) {
        ...
        $selector.remove();
        if($('.notification-inner-scroll:last').find('.listingnoti').length < 1) {
            $('.notification-inner-scroll:last').append('<p class="text-center" style="margin: 15px;">No Notifications Found</p>')
        }
    });
