        ...
        i.getElement().addEventListener('click', async function(event) {
            ...
            if(resp.data.length > 1) {
                ...
                resp.data.forEach((res, b) => {
                    html += `<a href="${domain}/listing-detail/${res.id}"><div class="location-thumbnaail location-image-btm"><div class="badge-icon-map">${res.listing_type === 'exclusive' ? 'Exclusive' : 'open'}</div><img style="height: 170px; width: 300px;" src="${domain}/${res.thumbnail}"><div class="price-wrapp price-wrap-location">
<p class="price" style="color:#223971">`+ '$'+res.rent.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,') +`</p><div class="additional-info">
 <ul><li><p>${res.bedrooms}</p>Beds</li><li><p>${res.baths}</p>Baths</li></ul>
                ...
                html += `<a href="${domain}/listing-detail/${resp.data[0].id}"><div class="location-thumbnaail location-image-btm"><div class="badge-icon-map">${resp.data[0].listing_type === 'exclusive' ? 'Exclusive' : 'Open'}</div><img style="height: 170px; width: 300px;" src="${domain}/${resp.data[0].thumbnail}"><div class="price-wrapp price-wrap-location">
            ...
            if(response && response.features[0].place_type) {
                response.features[0].place_type.forEach(res => {
                   if(required.includes(res)) {
                       ...
                       let boro = response.context[1];
                       let neighborhood = response.context[0];
                       let coords = response.geometry.coordinates;
                       let addressArray = response.place_name.split(',');

                       if(res === 'poi') {
                           streetAddress = addressArray[0].replace('.', '');
                           displayAddress = addressArray[1].replace('.', '');
                       } else {
                           streetAddress = addressArray[0].replace('.', '');
                           displayAddress = streetAddress +', New York';
                       }
